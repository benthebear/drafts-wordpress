=== Plugin Name ===
Contributors: benjamin.birkenhake
Tags: drafts, wp-admin, configurable
Requires at least: 4.1.1
Tested up to: 4.1.1
Stable tag: 4.1.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This Plugin adds a Button to the Admin-Menu that lead directly to the Post-Drafts,
as well a Dashboard-Widget, that lists a configurable Amount of recent Drafts.

== Description ==

I found it very hand to have a quick access to all Drafts, so this Plugins first purpose is
to add a Button to the Admin-Menü. This button contains a link to the Post-Admin selecting
only the Drafts.




== Installation ==


1. Upload the Folder `more-drafts` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress


== Frequently Asked Questions ==

= Can I use only the Dashboard and not the Admin-Menu Button? =

No, sorry, currently not.



== Screenshots ==

1. This screen shot description corresponds to screenshot-1.(png|jpg|jpeg|gif). Note that the screenshot is taken from
the /assets directory or the directory that contains the stable readme.txt (tags or trunk). Screenshots in the /assets 
directory take precedence. For example, `/assets/screenshot-1.png` would win over `/tags/4.3/screenshot-1.png` 
(or jpg, jpeg, gif).
2. This is the second screen shot

== Changelog ==

= 1.0 =
* Initial Version, containing all necessarry Features.



